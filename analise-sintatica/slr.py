    ...
    while True:
        ...
        for i in can:
            for x in g.getSymbols():
                goto_resp = goto(i, x, g)
                if len(goto_resp) > 0 and goto_resp not in can:
                    can.append(goto_resp)
    ...
    for state_l, l in enumerate(can):
        for i, item in enumerate(l):
            ...
            if dot_idx < len(rhs):
                ...
                if a in g.terminals:
                    goto_resp = goto(l, a, g)
                    if goto_resp in can:
                        j = can.index(goto_resp)
                ...
                if lhs != g.start_symbol:
                    ...
                    for aa in follow:
                        action_tab[state_l][aa].append(('reduce', (lhs, rhs)))
        ...
        for A in g.non_terminals:
            goto_resp = goto(l, A, g)
            if len(goto_resp) > 0 and goto_resp in can:
                j = can.index(goto_resp)
